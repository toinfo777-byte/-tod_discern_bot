...
__BOT_VERSION__ = "kb-1.7.3-three-pools-smart-autodetect"
...
REQUIRED_KEYS = {"id", "text", "options", "answer"}

def _looks_like_task_list(value) -> bool:
    if not isinstance(value, list) or not value:
        return False
    first = value[0]
    return isinstance(first, dict) and REQUIRED_KEYS.issubset(first.keys())

def _resolve_tasks_var(module, candidates: List[str]) -> List[dict]:
    # 1) пробуем известные имена
    for var in candidates:
        if hasattr(module, var):
            value = getattr(module, var)
            if _looks_like_task_list(value):
                return value
    # 2) автодетект: найдём любой список «как задачи»
    for name, val in vars(module).items():
        if _looks_like_task_list(val):
            logging.warning(
                f"[Auto-detect] В {module.__name__} найден пул задач в переменной '{name}'"
            )
            return val
    # 3) ничего не нашли — понятная ошибка
    raise ImportError(
        f"В модуле {module.__name__} нет подходящего пула задач. "
        f"Добавьте переменную вида list[dict] с ключами: {', '.join(REQUIRED_KEYS)} "
        f"(или назовите её как один из вариантов: {', '.join(candidates)})."
    )
